...
$$ U_{n+1}^{i} = U_{n}^{i} (1+ r \Delta t) - U_{n}^{i+1}\frac{\Delta t(r-q)}{2\Delta x} + U_{n}^{i-1}\frac{\Delta t(r - q - \sigma^{2})}{2\Delta x}, \forall I_{min}+1 \le i \le I_{max}-1$$
$$ U_{n+1}^{i} = aU_{n}^{i-1}+ bU_{n}^{i} + cU_{n}^{i+1} , \forall I_{min}+1 \le i \le I_{max}-1$$
for $a = \frac{\Delta t(r - q - \sigma^{2})}{2\Delta x}, b = 1+ r \Delta t, c = \frac{\Delta t(r-q)}{2\Delta x}$\\[3mm]
...
\setstretch{1.5}
	...
	$\Delta t = \dfrac{T}{N}$, 
	$\Delta x = \dfrac{x_{max}}{I}$, $I_{max} = I$, $I_{min} = -I$\;
	$a = \dfrac{\Delta t(r - q - \sigma^{2})}{2\Delta x}$\;
	$b = 1+ r \Delta t$\;
	$c = \dfrac{\Delta t(r-q)}{2\Delta x}$\;
	...
	\For {$j = N-1, N-2, \dots , 0$} {
		Generate a vector $F$ of length $(2I-1)$, with $F_{2I-1} = c\exp(-rn \Delta t)(S_{max} - X)$\;
		$U_{j} = A^{-1} (U_{j+1} + F)$\;
	}
